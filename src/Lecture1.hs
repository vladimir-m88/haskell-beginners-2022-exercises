...
lowerAndGreater n list =
  show n ++ " is greater than " ++ show gt
  ...
  where
    ...
    go l greater lower
      | null l = (greater, lower)
      | n > head l = go (tail l) (greater + 1) lower
      | n < head l = go (tail l) greater (lower + 1)
      | otherwise = go (tail l) greater lower
